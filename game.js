...
// --- DECLARAÇÃO DE VARIÁVEIS GLOBAIS E INSTÂNCIA DO JOGADOR ---
// Elas precisam estar aqui em cima para serem reconhecidas por todas as funções.
...
// Instância do jogador. Declarada aqui para que `player.draw()` funcione mesmo antes de initializeGame.
// As propriedades são resetadas em `initializeGame`.
let player = new Player(); // Vai criar o player com as configurações padrão.

const keys = {}; // Declara 'keys' aqui também, antes de ser usada no addEventListener

// 4. Teclas Pressionadas - Sistema de Input
window.addEventListener('keydown', (e) => {
    // Apenas registra teclas se o jogo já começou E NÃO estiver em game over
...
class Player {
    constructor() {
        ...
        this.calculateMaxHp(); // Garante que o HP máximo inicial seja calculado na criação
...
function updateUI() {
    // Garante que o player está definido antes de tentar acessar suas propriedades
    if (!player) return; 
...
function initializeGame() {
    // Cria uma NOVA instância de jogador, resetando tudo
    player = new Player(); 
...
function gameLoop() {
    ...
    if (!gameStarted || gameOver) {
        ...
        // Desenha o jogador na tela de "pausa"
        // (Ele já é instanciado globalmente, então `player` já existe)
        player.draw(); 
