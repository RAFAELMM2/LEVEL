...
window.addEventListener('keydown', (e) => {
    keys[e.key.toLowerCase()] = true; // Converte para minúsculas
});
...
function spawnEnemy() {
    if (gameOver || player.level >= BOSS_LEVEL) return; // Não spawna se o boss estiver ativo ou jogo acabou
...
function gameLoop() {
    if (gameOver) return; // Se o jogo acabou, para o loop

    gameFrame++; // Incrementa o contador de frames
...
// --- INICIAR O JOGO ---
updateUI(); // Atualiza a UI com os valores iniciais antes de começar o jogo
